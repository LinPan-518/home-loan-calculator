...
export const getClientAsync = createAsyncThunk<ClientData, void>("client/getClient", async () => {
  const response = await fetchJSON(`${process.env.PUBLIC_URL}/api/client.json`);
...
const clientSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getClientAsync.fulfilled, (state, action: PayloadAction<ClientData>) => {
        state.loading = false;
        state.client = action.payload;
        const { symbol, direction } = getCurrencySymbol(action.payload.currency); // Default to null if not found
